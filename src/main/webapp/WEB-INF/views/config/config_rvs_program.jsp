...
<script type="text/javascript">
    ...
    function selectDiv(){
    	document.listform.action ="/config_rvs_program.do";
					...
					<colgroup>
						...
						<thead>
							<tr class="text-small text-main-color2">
								<th style="border-right:1px solid #e0e3e7;"><input type="checkbox" id="delck"><label for="delck"><span></span></label></th>
								<th style="padding:10px 0;"><span>프로그램명</span></th>
								<th style="padding:10px 0;"><span>프로그램 파일명</span></th>
								<th style="padding:10px 0;"><span>파일경로</span></th>
								<th style="padding:10px 0;">
									<select id="ncp_div" name="ncp_div" onChange="selectDiv(this.value)" style="width:40px;">
										<option value="0">전체</option>
										<option value="4" <c:if test="${ncpDiv == 4}">selected</c:if>>내부</option>
										<option value="3" <c:if test="${ncpDiv == 3}">selected</c:if>>외부</option>
									</select>
								</th>
								<th style="padding:10px 0 0 4px;"><span>수정</span></th>
								<th style="padding:10px 0;"><span>삭제</span></th>
							</tr>
						</thead>
						<tbody>
						<c:forEach var="program" items="${programList}" varStatus="status">
							<tr class="text-medium text-main-color">
								...
								<td align="center" style="padding:25px 0;"><span><c:choose><c:when test="${program.ncp_div==4}"><spring:message code='config.inside'/></c:when><c:otherwise><spring:message code='config.outside'/></c:otherwise></c:choose></span></td>
			...
			<ul>
				...
				<li>
					...
					<sf:select path="ncp_div">
						<option value="4"><spring:message code='config.inside'/></option>
						<option value="3"><spring:message code='config.outside'/></option>
					</sf:select>
				</li>
			</ul>
			</sf:form>
