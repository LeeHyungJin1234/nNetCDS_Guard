					...
					<colgroup>
						...
						<thead>
							<tr class="text-small text-main-color2">
								...
								<th>
									<select onChange="document.getElementById('nler_risk_level').value=this.value;log_search();">
										<option value=""><spring:message code='log.risk'/></option>
										<option value="I" <c:if test="${ncEvtData.nler_risk_level eq 'I'}">selected</c:if>><spring:message code='log.info'/></option>
										<option value="W" <c:if test="${ncEvtData.nler_risk_level eq 'W'}">selected</c:if>><spring:message code='log.warning'/></option>
										<option value="R" <c:if test="${ncEvtData.nler_risk_level eq 'R'}">selected</c:if>><spring:message code='log.danger'/></option>
									</select>
								</th>
						...
						<tbody>
						    ...
						    <c:forEach var="mlog" items="${pginfo.result}" varStatus="status">
								<tr class="text-medium text-main-color <c:if test="${mlog.nler_risk_level eq '위험'}">fail</c:if>">
									<td align="center"><span><c:out value="${pginfo.totalArticle-((pginfo.currentPage-1)*pginfo.perArticle)-status.index}" /></span></td>
									<td align="center"><span>${mlog.nler_create_date}</span></td>
									<td align="center"><span>${mlog.nler_risk_level}</span></td>
									<td align="center" style="word-break:break-all;"><span>${mlog.ncp_name}</span></td>
									<td style="word-break:break-all;text-align:left"><span>${mlog.nler_message}</span></td>
								</tr>
							</c:forEach>
							</c:when>
							<c:otherwise>
