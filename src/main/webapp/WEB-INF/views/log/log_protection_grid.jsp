...
<script type="text/javascript">
	function log_search() {   
		...
		var temp = calDateRange(form.nlpt_create_sdate.value, form.nlpt_create_edate.value);
		if(temp<0){
			alert("<spring:message code='log.dategreater'/>");
			return;
		}
		form.action="protection_grid_search.do";
		form.submit();
	}
				...
				<div class="top-section log-section">
					<form id="search" method="get">
						...
						<ul id="search" class="clearfix">
							...
							<li><span><spring:message code='stat.searchperiod'/></span><input type="text" id="from" name="nlpt_create_sdate" class="input date" value="${ncEvtData.nlpt_create_sdate}"><img src="/img/search-bar.png"><input type="text" class="input date"  id="to" name="nlpt_create_edate" value="${ncEvtData.nlpt_create_edate}"></li>
...
<script>
	...
	const dataGrid = $('#gridContainer').dxDataGrid({
	    ...
	    columns: [{
		  ...
		  dataField: 'nlpt_result',
		  caption: '작업 결과',
		  alignment: 'center',
		  dataType: 'String',
		  width: 127,
		 }, {
		  dataField: 'nlpt_risk_level',
		  caption: '위험도',
		  alignment: 'center',
		  dataType: 'String',
		  width: 127,
		  cellTemplate(container, options) {
	          container.parent().addClass((options.data.nlpt_risk_level == '위험') ? 'inc' : 'dec');
	          container.html(options.text);
	        }
		 },{
	      dataField: 'nlpt_prog_name',
	      caption: '작업 구분',
	      alignment: 'center',
	      dataType: 'String',
	      width: 242,
	    }, {
		  dataField: 'nlpt_message',
		  caption: '세부내용',
		  alignment: 'center',
		  dataType: 'String',
		  width: 500,
