...
decimals="0" showborder="0" plotSpacePercent="50">
	...
	%>
			...
			<c:forEach var="send" items="${sList}" varStatus="status">
				<c:set var="compare"><%=new java.text.SimpleDateFormat("yyyy-MM-dd").format(c1.getTime())%></c:set>
	
				<c:if test="${send.nlp_create_date == compare}">
					<set value="${send.cnt}" />
	...
	%>
			<c:set var="temp">0</c:set>
			<c:forEach var="receive" items="${rList}" varStatus="status">
				<c:set var="compare"><%=new java.text.SimpleDateFormat("yyyy-MM-dd").format(c1.getTime())%></c:set>
				
				<c:if test="${receive.nlp_create_date == compare}">
					<set value="${receive.cnt}" />
					<c:set var="temp">1</c:set>
				</c:if>
