...
<script type="text/javascript">
	function log_search() {   
		...
		var temp = calDateRange(form.nle_create_sdate.value, form.nle_create_edate.value);
		if(temp<0){
			alert("<spring:message code='log.dategreater'/>");
			return;
		}
		$('.wrap-loading').removeClass('display-none');
		form.action="event_search.do";
		form.submit();
	}
		...
		<div id="right-content">
			<div class="right-wrapper">
				<div class="top-section log-section">
					...
					<input type="hidden" name="log_type" value="event" />
					<input type="hidden" name="count" value="${pginfo.totalArticle}" />
					<input type="hidden" name="datetab" id="datetab" value="${ncEvtData.datetab}" />
					<input type="hidden" name="nle_risk_level" id="nle_risk_level" />
						<ul id="search" class="clearfix">
							<!-- <li><span>통신방향</span>
								<select name="nle_div" id="nle_div" onChange="log_search()" <c:if test="${Config.getInstance().getModel() eq 'single'}">disabled=true</c:if>>
									<option value="1" <c:if test="${ndEvtData.nle_div == '1'}" > selected </c:if>><spring:message code='config.inside'/> → <spring:message code='config.outside'/></option>
									<option value="3" <c:if test="${ndEvtData.nle_div == '3'}" > selected </c:if>><spring:message code='config.outside'/> → <spring:message code='config.inside'/></option>
								</select>							
							</li> -->
							<li><span><spring:message code='stat.searchperiod'/></span><input type="text" id="from" name="nle_create_sdate" class="input date" value="${ncEvtData.nle_create_sdate}"><img src="/img/search-bar.png"><input type="text" class="input date" id="to" name="nle_create_edate" value="${ncEvtData.nle_create_edate}"></li>
					...
					<colgroup>
						...
						<thead>
							<tr class="text-small text-main-color2">
								...
								<th>
									<select onChange="document.getElementById('nle_risk_level').value=this.value;log_search();">
										<option value=""><spring:message code='log.risk'/></option>
										<option value="I" <c:if test="${ncEvtData.nle_risk_level eq 'I'}">selected</c:if>><spring:message code='log.info'/></option>
										<option value="W" <c:if test="${ncEvtData.nle_risk_level eq 'W'}">selected</c:if>><spring:message code='log.warning'/></option>
										<option value="R" <c:if test="${ncEvtData.nle_risk_level eq 'R'}">selected</c:if>><spring:message code='log.danger'/></option>
									</select>
								</th>
						...
						<tbody>
						    ...
						    <c:forEach var="mlog" items="${pginfo.result}" varStatus="status">
								<tr class="text-medium text-main-color <c:if test="${mlog.nle_risk_level eq '위험'}">fail</c:if>">
									<td align="center"><span><c:out value="${pginfo.totalArticle-((pginfo.currentPage-1)*pginfo.perArticle)-status.index}" /></span></td>
									<td align="center"><span>${mlog.nle_create_date}</span></td>
									<td align="center"><span>${mlog.nle_risk_level}</span></td>
									<td align="center"><span>${mlog.npl_name}<c:if test="${mlog.npl_name eq null}">${mlog.nle_prog_name}</c:if></span></td>
									<td align="left"><span>${mlog.nle_message}</span></td>
								</tr>
							</c:forEach>
							</c:when>
							<c:otherwise>
