	    	...
	    	$.ajax({        
				...
				success: function(data) {
					if(data == "true"){
						...
						location.replace('/forward_policy.do');
					}else{
						alert(data);
					}
				},
...
<script type="text/javascript">
    ...
    function writeCheck() {
		...
		if( check ){
	    	if(confirm("<spring:message code='alert.register'/>")){
	    		...
	    		$(".nlp_src_ip").attr("disabled", false); // 값 넘기기 위해 disabled 풀어줌
	    		$(".nlp_src_port").attr("disabled", false); // 값 넘기기 위해 disabled 풀어줌
	    		$(".nlp_dst_ip").attr("disabled", false); // 값 넘기기 위해 disabled 풀어줌
	    		$(".nlp_dst_port").attr("disabled", false); // 값 넘기기 위해 disabled 풀어줌

	    		if(form.sip.value=="any") form.nlp_src_ip.value=form.sip.value;
	    		if(form.sport.value=="any") form.nlp_src_port.value=form.sport.value;
	    		if(form.dip.value=="any") form.nlp_dst_ip.value=form.dip.value;
	    		if(form.dport.value=="any") form.nlp_dst_port.value=form.dport.value;
    ...
    function change_disable(name, value){
    	if(value=="any"){ // 모든
...
<body>
					...
					<table class="scroll">
						<thead>
							<tr class="text-small text-main-color2">
								...
								<th class="group2"><span>정책명</span></th>
								<th style="border-right:1px solid #e0e3e7;" colspan="4"><span>일방향 전송정책</span></th>
						...
						<c:forEach var="policy" items="${policyList}" varStatus="status">
							<tr class="text-medium text-main-color">
								...
								<td class="list-group2">
									...
									<c:otherwise>
										<c:choose>
										<c:when test="${'Packets' eq policy.ntc_name}">
											<img src="/img/icon_snmp.png" <c:if test="${policy.nlp_use_yn==0}">class="opacity2"</c:if>>
										</c:when>
										<c:otherwise>
									...
									<span <c:if test="${policy.nlp_use_yn==0}">class="opacity4"</c:if>>${policy.nlp_name}(${policy.ndc_name}<c:if test="${policy.ndc_name==null}">${policy.npe_name}</c:if>)</span>
								</td>
							<c:choose>
							<c:when test="${'6' eq policy.ntc_seq}">
								<td align="center" class="list-group3">
									<span <c:if test="${policy.nlp_use_yn==0}">class="opacity4"</c:if>><font size="2px" color="#b3b5b7">내부 NIC 번호</font><br>${policy.nlp_tx_nic}</span>
								</td>
								<td align="center" style="vertical-align:middle;width:260px" colspan="2"><span <c:if test="${policy.nlp_use_yn==0}">class="opacity4"</c:if>>
									<font size="2px" color="#b3b5b7">Oneway 포트</font><br><c:choose><c:when test="${'1' eq policy.nlp_oneway_port}">Master</c:when><c:otherwise>Slave</c:otherwise></c:choose></span></td>
								<td align="center" class="list-group6">
									<span <c:if test="${policy.nlp_use_yn==0}">class="opacity4"</c:if>><font size="2px" color="#b3b5b7">외부 NIC 번호</font><br>${policy.nlp_rx_nic}</span>
								</td>
							</c:when>
							<c:otherwise>
								<td align="center" class="list-group3">
									<span <c:if test="${policy.nlp_use_yn==0}">class="opacity4"</c:if>>
									<font size="2px" color="#b3b5b7">출발지 IP</font><br><c:forEach var="route" items="${routeList}" begin="${status.index}" end="${status.index}">
								...
								<td align="center" class="list-group4"><span <c:if test="${policy.nlp_use_yn==0}">class="opacity4"</c:if>><font size="2px" color="#b3b5b7">출발지 수신 포트</font><br>${policy.nlp_rcv_port}</span></td>
								<td align="center" class="list-group5"><span <c:if test="${policy.nlp_use_yn==0}">class="opacity4"</c:if>><font size="2px" color="#b3b5b7">일방향 전송 포트</font><br>${policy.nlp_trans_port}</span></td>
								<td align="center" class="list-group6">
									<span <c:if test="${policy.nlp_use_yn==0}">class="opacity4"</c:if>>
									<font size="2px" color="#b3b5b7">도착지 IP:PORT</font><br><c:forEach var="route" items="${routeList}" begin="${status.index}" end="${status.index}">
								...
								<td align="center" class="list-group10"><span <c:if test="${policy.nlp_use_yn==0}">class="opacity4"</c:if>>${policy.ncp_name}<font size="2px" color="#b3b5b7">(내부)</font><br>${policy.rx_ncp_name}<font size="2px" color="#b3b5b7">(외부)</font></span></td>
	...
	<div id="wripopup" class="popup" style="display:none;">
		...
		<div class="pop-wrapper">
			...
			<sf:hidden path="nlp_div" value="1" /> <!-- 정/역방향 구분 -->
			...
			<div class="left-content">
				<ul>
					...
					<li class="ether" style="display:none">
						...
						<sf:select path="npe_value">
							<c:forEach var="ether" items="${etherType}">
								<option value="${ether.npe_value}">${ether.npe_name}</option>
							</c:forEach>
						</sf:select>
					</li>
					<li class="ip_proto" style="display:none">
						<label>프로토콜 타입</label><img src="/img/info.png" style="margin-left:2px;vertical-align:middle" onClick="window.open('/desc_popup.do?type=2', 'proto', 'width=750px,height=650px,scrollbars=no,resizable=no,toolbars=no,menubar=no,location=no');">
						<sf:select path="npp_no">
							<c:forEach var="ipproto" items="${ipProtocol}">
								<option value="${ipproto.npp_no}">${ipproto.npp_name}</option>
							</c:forEach>
						</sf:select>
					</li>
					<li class="src_ip" style="display:none">
						<label>출발지 IP</label>
						<select name="sip" style="width:95px" onChange="change_disable('nlp_src_ip', this.value);">
							<option value="any">모든</option>
							<option value="sel">특정</option>
						</select> <sf:input path="nlp_src_ip" class="nlp_src_ip" style="width:183px" disabled="true" />
					</li>
					<li class="src_port" style="display:none">
						<label>출발지 포트</label>
						<select name="sport" style="width:95px" onChange="change_disable('nlp_src_port', this.value);">
							<option value="any">모든</option>
							<option value="sel">특정</option>
						</select> <sf:input path="nlp_src_port" class="nlp_src_port" style="width:183px" disabled="true" />
					</li>
					<li class="dst_ip" style="display:none">
						<label>도착지 IP</label>
						<select name="dip" style="width:95px" onChange="change_disable('nlp_dst_ip', this.value);">
							<option value="any">모든</option>
							<option value="sel">특정</option>
						</select> <sf:input path="nlp_dst_ip" class="nlp_dst_ip" style="width:183px" disabled="true" />
					</li>
					<li class="dst_port" style="display:none">
						<label>도착지 포트</label>
						<select name="dport" style="width:95px" onChange="change_disable('nlp_dst_port', this.value);">
							<option value="any">모든</option>
							<option value="sel">특정</option>
						</select> <sf:input path="nlp_dst_port" class="nlp_dst_port" style="width:183px;" disabled="true" />
					</li>
					<li>
			...
			<div class="right-content packets" style="display:none">
				<div class="path-wrapper1">
					<h2>경로</h2>
					<ul class="box box1_1">
						<li>
							<label style="font-size:13px;">내부 NIC 번호</label><br>
							<sf:input path="nlp_tx_nic" style="width:208px" />
						</li>
						<li>
							<label style="font-size:13px;">Oneway 포트</label><br/>
							<sf:select path="nlp_oneway_port">
						...
						<li style="border-right: none;">
							...
							<sf:input path="nlp_rx_nic" style="width:208px" />
						</li>
					</ul>
				</div>
			</div>
